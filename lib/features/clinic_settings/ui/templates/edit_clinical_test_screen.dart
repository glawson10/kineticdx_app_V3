...
class EditClinicalTestScreen extends StatefulWidget {
  final String clinicId;
  final ClinicalTestRegistryItem? existing;

  const EditClinicalTestScreen({
    super.key,
  ...
  @override
  State<EditClinicalTestScreen> createState() => _EditClinicalTestScreenState();
}

class _EditClinicalTestScreenState extends State<EditClinicalTestScreen> {
  ...
  final _interpretation = TextEditingController();

  final _resultType = TextEditingController();
  final _allowedResults = TextEditingController();
  ...
  void initState() {
    ...
    if (e != null) {
      ...
      _allowedResults.text = 'positive, negative, notTested';
      _active = true;
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await _repo.upsertClinicalTest(
        ...
        positiveCriteria: _positive.text.trim().isEmpty ? null : _positive.text.trim(),
        contraindications: _contra.text.trim().isEmpty ? null : _contra.text.trim(),
        interpretation: _interpretation.text.trim().isEmpty ? null : _interpretation.text.trim(),
        resultType: _resultType.text.trim().isEmpty ? null : _resultType.text.trim(),
      ...
      body: ListView(
        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'Category (e.g. special_test)'),
          ),
          const SizedBox(height: 8),
          TextField(
            controller: _regions,
            decoration: const InputDecoration(
              ...
              helperText: 'Example: cervical, shoulder, lumbar',
            ),
          ),
          const SizedBox(height: 8),
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Tags (csv)',
              helperText: 'Example: radiculopathy, provocation',
            ),
          ),
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              ...
              helperText: 'Default: positive, negative, notTested',
            ),
          ),
        ],
      ),
