...
async function updateEpisode(req) {
    ...
    if (!clinicId || !patientId || !episodeId) {
        throw new https_1.HttpsError("invalid-argument", "clinicId, patientId, episodeId required.");
    }
    const db = admin.firestore();
    const uid = req.auth.uid;
    await (0, authz_1.requireActiveMemberWithPerm)(db, clinicId, uid, "clinical.write");
    const episodeRef = db.doc(`clinics/${clinicId}/patients/${patientId}/episodes/${episodeId}`);
    const snap = await episodeRef.get();
    if (!snap.exists)
        throw new https_1.HttpsError("not-found", "Episode not found.");
    const existing = snap.data();
    if ((existing === null || existing === void 0 ? void 0 : existing.status) !== "open") {
        throw new https_1.HttpsError("failed-precondition", "Cannot edit a closed episode.");
    }
    ...
    if (((_g = req.data) === null || _g === void 0 ? void 0 : _g.title) !== undefined)
        update.title = req.data.title;
    if (((_h = req.data) === null || _h === void 0 ? void 0 : _h.primaryComplaint) !== undefined)
        update.primaryComplaint = req.data.primaryComplaint;
    if (((_j = req.data) === null || _j === void 0 ? void 0 : _j.onsetDate) !== undefined)
        update.onsetDate = req.data.onsetDate;
    if (((_k = req.data) === null || _k === void 0 ? void 0 : _k.referralSource) !== undefined)
        update.referralSource = req.data.referralSource;
    if (((_l = req.data) === null || _l === void 0 ? void 0 : _l.assignedPractitionerId) !== undefined)
        update.assignedPractitionerId = req.data.assignedPractitionerId;
    if (((_m = req.data) === null || _m === void 0 ? void 0 : _m.tags) !== undefined)
...
//# sourceMappingURL=updateEpisode.js.map