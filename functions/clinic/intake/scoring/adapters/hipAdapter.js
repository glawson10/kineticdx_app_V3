...
function buildHipLegacyAnswers(answers) {
    ...
    const sideToken = (s) => typeof s === "string" && s.startsWith("side.") ? s.substring("side.".length) : "";
    // Onset (canonical: onset.sudden/gradual/unsure) → legacy expects raw "sudden/gradual/unsure"
    const onsetToken = (s) => typeof s === "string" && s.startsWith("onset.") ? s.substring("onset.".length) : "";
    // Pain location (canonical single) → legacy H_where is MULTI with scorer tokens.
    const whereTokens = (loc) => {
    ...
    const aggsTokens = (arr) => {
        const a = Array.isArray(arr) ? arr.map(String) : [];
        const out = [];
        for (const opt of a) {
            if (opt === "aggs.stairs")
                out.push("stairs");
            if (opt === "aggs.sideLying")
                out.push("side-lying"); // scorer checks "side-lying"
            if (opt === "aggs.standWalk")
                out.push("stand_walk"); // scorer checks "stand_walk"
    ...
    const sleepToken = (s) => {
        if (s === "sleep.wakesSide")
            return "wakes_side"; // scorer checks sleep === "wakes_side"
        if (s === "sleep.none")
            return "none";
        if (s === "sleep.wakesOther")
            return "wakes_other";
        return "";
    };
    ...
    const irritOnToken = (s) => typeof s === "string" && s.startsWith("irritOn.") ? s.substring("irritOn.".length) : "";
    const irritOffToken = (s) => typeof s === "string" && s.startsWith("irritOff.") ? s.substring("irritOff.".length) : "";
    // Amber risks multi → strip prefix "risks."
    const amberRiskTokens = (arr) => {
        const a = Array.isArray(arr) ? arr.map(String) : [];
        if (a.includes("risks.none"))
            return [];
        return a
    ...
    return [
        ...
        // -------------------------
        single("H_additionalInfo", (_a = v("hip.history.additionalInfo")) !== null && _a !== void 0 ? _a : ""),
    ];
}
//# sourceMappingURL=hipAdapter.js.map