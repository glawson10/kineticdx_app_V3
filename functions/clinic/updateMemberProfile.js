...
async function getMembershipWithFallback(params) {
    var _a, _b;
    const canonRef = db.collection("clinics").doc(params.clinicId).collection("memberships").doc(params.uid);
    const canonSnap = await canonRef.get();
    if (canonSnap.exists)
        return ((_a = canonSnap.data()) !== null && _a !== void 0 ? _a : {});
    const legacyRef = db.collection("clinics").doc(params.clinicId).collection("members").doc(params.uid);
...
async function updateMemberProfile(req) {
    ...
    const displayName = safeStr((_c = req.data) === null || _c === void 0 ? void 0 : _c.displayName);
    if (!clinicId)
    ...
    const canonRef = db.collection("clinics").doc(clinicId).collection("memberships").doc(memberUid);
    const legacyRef = db.collection("clinics").doc(clinicId).collection("members").doc(memberUid);
