...
function computeTriage(a) {
    const notes = [];
    let triage = "green";
    if (a.redFlags.fever_or_hot_red_joint === "yes" ||
...
function score(a, triage) {
    ...
    const limitedOverhead = limits.includes("Reaching overhead") || limits.includes("Sports/overhead work");
    ...
    if (a.stiffness === "yes" || limitedJacket) {
        S.rc_tendinopathy.score -= 0.18;
        S.rc_tendinopathy.why.push("Stiffness/jacket limitation reduces likelihood of isolated RC tendinopathy");
    }
    // ───────── Full thickness tear ─────────
    ...
    if (a.redFlags.can_active_elevate === "no") {
        ...
        S.full_thickness_rc_tear.why.push("Cannot actively elevate suggests large tear/structural disruption");
        S.rc_tendinopathy.score -= 0.12;
        S.rc_tendinopathy.why.push("Inability to elevate reduces likelihood of simple RC tendinopathy");
    }
    if (a.weakness === "yes" && a.redFlags.can_active_elevate === "no") {
        S.full_thickness_rc_tear.score += 0.45;
        S.full_thickness_rc_tear.why.push("Weakness + inability to elevate supports full-thickness tear pattern");
    }
    // ───────── Adhesive capsulitis ─────────
    if (a.stiffness === "yes") {
    ...
    if (a.clicking === "yes" && a.onset === "minorFallJar") {
        ...
        S.rc_tendinopathy.why.push("Clicking after jar-type onset reduces likelihood of isolated RC tendinopathy");
    }
    ...
    if (flareOnset &&
        ...
        S.calcific_bursitis.why.push("Acute flare pattern: night + overhead pain without true stiffness, still able to elevate");
        // reduce RC slightly in this flare context
        S.rc_tendinopathy.score -= 0.16;
        S.rc_tendinopathy.why.push("Flare pattern reduces likelihood of simple RC tendinopathy");
    }
    // ───────── Cervical ─────────
    ...
    if (tenderSpot === "ac_point") {
        ...
        S.biceps_slap.why.push("AC point tenderness reduces likelihood of primary biceps source");
        S.rc_tendinopathy.score -= 0.08;
        S.rc_tendinopathy.why.push("AC point tenderness reduces likelihood of isolated RC tendinopathy");
    }
    if (tenderSpot === "bicipital_groove") {
        S.biceps_slap.score += 0.30;
        S.biceps_slap.why.push("Bicipital groove tenderness supports biceps/SLAP spectrum");
        S.ac_joint.score -= 0.10;
        S.ac_joint.why.push("Bicipital groove tenderness reduces likelihood of primary AC source");
    }
    return S;
}
/* ----------------- summary ----------------- */
