"use strict";
// functions/src/cervical/cervicalScoring.ts
//
// Pure scoring + summary builder for Cervical Spine.
// This file holds the EXACT scoring logic (lifted from processCervicalAssessment.ts)
// so it can be reused by callables AND clinic/intake engines without duplication.
Object.defineProperty(exports, "__esModule", { value: true });
exports.computeCervicalTriage = computeCervicalTriage;
exports.scoreCervical = scoreCervical;
...
/* ----------------- triage ----------------- */
function computeCervicalTriage(answers) {
    const notes = [];
    let triage = "green";
    ...
    // conservative myelopathy-style escalation
...
/* ----------------- scoring ----------------- */
function scoreCervical(answers, triage) {
    var _a, _b;
    ...
    const painLocation = ((_a = getOne(answers, "C_pain_location")) !== null && _a !== void 0 ? _a : "central"); // central|one_side|both_sides
    const intoShoulder = yn(getOne(answers, "C_pain_into_shoulder"));
    const belowElbow = yn(getOne(answers, "C_pain_below_elbow"));
    const vasWorst = (_b = getSlider(answers, "C_vas_worst")) !== null && _b !== void 0 ? _b : 0;
    const nightPain = yn(getOne(answers, "C_night_pain"));
    const morningStiff30 = yn(getOne(answers, "C_morning_stiff_30"));
    ...
    const headacheBetterNeckCare = yn(getOne(answers, "C_headache_better_with_neck_care"));
    const visualDisturbance = yn(getOne(answers, "C_visual_disturbance"));
    const gaitUnsteady = yn(getOne(answers, "C_gait_unsteady"));
    ...
    if (coughSneezeWorse) {
        S.cervical_radiculopathy.score += 0.1;
    ...
    if (!belowElbow && !armTingling && !armWeakness && !headache) {
        S.mechanical_neck_pain.score += 0.1;
    ...
    const anyTraumaSignal = yn(getOne(answers, "C_rf_high_speed_crash")) ||
    ...
    if (neckMovtWorse && headache) {
        ...
        S.myofascial_trigger_points.why.push("Neck pain + headache (suboccipital/upper trap referral possible)");
    }
    /* -------- Shoulder masquerader (tuned) -------- */
    if (intoShoulder && !neckMovtWorse && !armTingling && !armWeakness && !belowElbow) {
        S.shoulder_referred.score += 0.26;
        S.shoulder_referred.why.push("Shoulder/upper arm pain without neck modulation or neuro signs");
    }
...
/* ----------------- summary builder ----------------- */
function buildCervicalSummary(answers) {
    const { triage, notes } = computeCervicalTriage(answers);
    const scored = scoreCervical(answers, triage);
...
//# sourceMappingURL=cervicalScoring.js.map