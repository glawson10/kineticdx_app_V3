...
exports.renderGeneralVisitV1Html = renderGeneralVisitV1Html;
function safeStr(v) {
...
function readMulti(answers, qid) {
    ...
    if (a && typeof a === "object" && a.t === "multi" && Array.isArray(a.v)) {
        return a.v.map((x) => safeStr(x)).filter(Boolean);
    }
    return [];
}
function readBool(answers, qid) {
    const a = answers[qid];
    if (a && typeof a === "object" && a.t === "bool")
        return a.v === true;
    return null;
}
function yn(v) {
...
function renderGeneralVisitV1Html(input) {
    const { clinicName, sessionId, submittedAt, schemaVersion, clinicLogoUrl, patient, answers, } = input;
    const reason = readText(answers, "generalVisit.goals.reasonForVisit");
    const clarity = labelSingle(readSingle(answers, "generalVisit.meta.concernClarity"), concernClarityLabels);
    const areas = labelsMulti(readMulti(answers, "generalVisit.history.bodyAreas"), bodyAreaLabels);
    const impact = labelSingle(readSingle(answers, "generalVisit.function.primaryImpact"), primaryImpactLabels);
    const limited = labelsMulti(readMulti(answers, "generalVisit.function.limitedActivities"), limitedActivityLabels);
    const duration = labelSingle(readSingle(answers, "generalVisit.history.duration"), durationLabels);
    const intent = labelsMulti(readMulti(answers, "generalVisit.goals.visitIntent"), visitIntentLabels);
    ...
    <div style="max-width:760px; margin:0 auto; background:#ffffff; border-radius:10px; overflow:hidden; border:1px solid #e5e7eb;">
      ...
      <div style="padding:20px 24px; border-bottom:1px solid #e5e7eb;">
        <div style="display:flex; align-items:center; justify-content:space-between; gap:16px;">
          ...
          ${clinicLogoUrl
        ? `<img src="${escapeHtml(clinicLogoUrl)}" alt="${escapeHtml(clinicName)}" style="max-height:44px; object-fit:contain;" />`
        : ""}
        </div>
      </div>
      ...
      <div style="padding:18px 24px;">
        ...
        <table style="width:100%; border-collapse:collapse; font-size:12px;">
          <tr><td style="padding:6px 0; color:#6b7280; width:180px;">Clinic</td><td style="padding:6px 0;">${escapeHtml(clinicName)}</td></tr>
          <tr><td style="padding:6px 0; color:#6b7280;">Intake session ID</td><td style="padding:6px 0;">${escapeHtml(sessionId)}</td></tr>
          <tr><td style="padding:6px 0; color:#6b7280;">Submitted at</td><td style="padding:6px 0;">${escapeHtml(submittedAtLocal)}</td></tr>
      ...
      <div style="padding:0 24px 18px;">
        ...
        <table style="width:100%; border-collapse:collapse; font-size:12px;">
          <tr><td style="padding:6px 0; color:#6b7280; width:180px;">Name</td><td style="padding:6px 0;">${escapeHtml(patient.fullName || "—")}</td></tr>
          <tr><td style="padding:6px 0; color:#6b7280;">Email</td><td style="padding:6px 0;">${escapeHtml(patient.email || "—")}</td></tr>
          <tr><td style="padding:6px 0; color:#6b7280;">Phone</td><td style="padding:6px 0;">${escapeHtml(patient.phone || "—")}</td></tr>
        </table>
      </div>
      ...
      <div style="padding:0 24px 18px;">
        ...
        <div style="padding:12px; border:1px solid #e5e7eb; border-radius:10px; margin-bottom:10px;">
          ...
          <div style="margin-top:6px; font-size:12px; white-space:pre-wrap;">${escapeHtml(reason || "—")}</div>
        ...
        <div style="padding:12px; border:1px solid #e5e7eb; border-radius:10px; margin-bottom:10px;">
          ...
          <div style="margin-top:6px; font-size:12px;">${escapeHtml(joinOrDash(areas))}</div>
        ...
        <div style="padding:12px; border:1px solid #e5e7eb; border-radius:10px; margin-bottom:10px;">
          ...
          <div style="margin-top:6px; font-size:12px;">${escapeHtml(joinOrDash(limited))}</div>
        ...
        <div style="padding:12px; border:1px solid #e5e7eb; border-radius:10px;">
          ...
          <div style="margin-top:6px; font-size:12px;">${escapeHtml(joinOrDash(intent))}</div>
      ...
      <div style="padding:0 24px 22px;">
        ...
        <table style="width:100%; border-collapse:collapse; font-size:12px;">
          <tr><td style="padding:6px 0; color:#6b7280; width:360px;">Not an emergency (acknowledged)</td><td style="padding:6px 0;">${escapeHtml(ackNE)}</td></tr>
          <tr><td style="padding:6px 0; color:#6b7280;">No diagnosis from this questionnaire (acknowledged)</td><td style="padding:6px 0;">${escapeHtml(ackND)}</td></tr>
        </table>
...
//# sourceMappingURL=generalVisitV1Renderer.js.map