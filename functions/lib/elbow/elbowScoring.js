...
exports.computeElbowTriage = computeElbowTriage;
exports.buildElbowSummaryFromLegacyAnswers = buildElbowSummaryFromLegacyAnswers;
const diffs = {
    lat_epi: {
        key: "lat_epi",
...
// ---------- helpers ----------
function firstSingle(answers, id) {
    var _a;
    const a = answers.find((x) => x.id === id && x.kind === "single");
    return (_a = a === null || a === void 0 ? void 0 : a.value) !== null && _a !== void 0 ? _a : "";
}
function firstSlider(answers, id) {
    const a = answers.find((x) => x.id === id && x.kind === "slider");
    return typeof (a === null || a === void 0 ? void 0 : a.value) === "number" ? a.value : 0;
}
function multiVals(answers, id) {
    const a = answers.find((x) => x.id === id && x.kind === "multi");
    return Array.isArray(a === null || a === void 0 ? void 0 : a.values) ? a.values : [];
}
// ---------- triage ----------
function computeElbowTriage(answers) {
    const notes = [];
    let triage = "green";
    ...
    if (injuryHigh || deformity || rapidSwelling || numbAfter || cantFullyStraighten) {
        if (injuryHigh)
            notes.push("High-force injury");
        if (deformity)
            notes.push("Visible deformity");
        if (rapidSwelling)
            notes.push("Rapid swelling after injury");
        if (numbAfter)
            notes.push("Numbness/tingling after injury");
        if (cantFullyStraighten)
            notes.push("Cannot fully straighten elbow");
        triage = "red";
    }
    ...
    if (triage !== "red" && (nightPain || constantPain || swellingNow)) {
        if (nightPain)
            notes.push("Night pain");
        if (constantPain)
            notes.push("Constant pain");
        if (swellingNow)
            notes.push("Swelling present");
        triage = "amber";
    }
    return { triage, notes };
}
...
// ---------- build summary ----------
function buildElbowSummaryFromLegacyAnswers(answers) {
...
//# sourceMappingURL=elbowScoring.js.map