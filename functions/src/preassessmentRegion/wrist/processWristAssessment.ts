...
function score(answers: Answer[], triage: Triage) {
  ...
  // Core inputs (wrist)
  const part = (getOne(answers, "H_part") ?? "wrist").toLowerCase(); // wrist | thumb | finger
  ...
  const digit = (getOne(answers, "H_digit") ?? "").toLowerCase(); // thumb/index/middle/ring/little
  const joint = (getOne(answers, "H_joint") ?? "").toLowerCase(); // dip/pip/mcp/cmc
  ...
  if (isFinger) {
    ...
    ["tfcc", "ecu", "lt_ligament", "ulnar_impaction", "intersection", "carpal_boss", "kienbock"].forEach(
      (k) => (S[k as Differential].score -= 0.14)
    );
  }

  if (isThumb) {
    // Keep DeQ + thumb CMC, reduce deep ulnar wrist instability unless where supports
    ["lt_ligament", "kienbock"].forEach((k) => (S[k as Differential].score -= 0.10));
  }
  ...
  if (isWrist) {
    ...
    [
      ...
      "glomus_tumour",
      "digital_nerve",
    ].forEach((k) => (S[k as Differential].score -= 0.10));
  }
  ...
  if (!isFinger) {
    if (!W.has("ulnar")) {
      ["tfcc", "ecu", "lt_ligament", "ulnar_impaction"].forEach((k) => (S[k as Differential].score -= 0.12));
    }
    if (!W.has("radial")) {
      ["de_quervain", "intersection", "radial_nerve"].forEach((k) => (S[k as Differential].score -= 0.12));
    }
    if (!W.has("dorsal")) {
      ["ganglion", "carpal_boss", "kienbock", "intersection"].forEach((k) => (S[k as Differential].score -= 0.06));
    }
    if (!W.has("volar")) {
  ...
  /* ----------------- “severity bump” (light) ----------------- */
  if (cantGrip) {
    ["tfcc", "ulnar_impaction", "de_quervain", "wrist_oa", "trigger_finger", "finger_oa"].forEach(
      (k) => (S[k as Differential].score += 0.04)
    );
  }
...
function buildSummary(answers: Answer[]) {
  ...
  const part = (getOne(answers, "H_part") ?? "wrist").toLowerCase();
  const side = getOne(answers, "W_side") ?? "";
  const sideLabel = side ? `Hand (${side})` : "Hand";
  const partLabel =
    part === "finger" ? "Fingers" : part === "thumb" ? "Thumb" : "Wrist";
...
export async function processWristAssessmentCore(
  data: any,
