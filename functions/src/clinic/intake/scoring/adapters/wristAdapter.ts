...
/**
 * Legacy answer shapes expected by processWristAssessment.ts
 */
type Answer =
 ...
 */
export function buildWristLegacyAnswers(
  ...
  const multi = (id: string, values: string[]): Answer => ({
    id,
    kind: "multi",
    values,
  });

  const slider = (id: string, value: unknown): Answer => ({
    id,
    kind: "slider",
  ...
  const noneIfEmpty = (arr: string[]) => (arr.length ? arr : ["none"]);
  ...
  const acuteInjury = String(v("wrist.redflags.acuteInjury") ?? "");
  const Q2_injury =
    acuteInjury === "injury.yes" ? "yes" : acuteInjury === "injury.no" ? "no" : "";
  ...
  const injClusterRaw =
    Q2_injury === "yes"
      ...
      : [];
  const injuryCluster = noneIfEmpty(
    injClusterRaw
      .map((id) => stripPrefix(id, "inj."))
      .filter((tok) =>
        ["no_weight_bear", "pop_crack", "deformity", "severe_pain", "immediate_numb"].includes(tok)
      )
  );
  ...
  const side = String(v("wrist.context.side") ?? "");
  const Q_side =
    side === "side.left"
      ? "Left"
      : side === "side.right"
  ...
  const zoneRaw = String(v("wrist.symptoms.zone") ?? "");
  const Q4_zone = (() => {
  ...
  const onsetRaw = String(v("wrist.history.onset") ?? "");
  const Q5_onset = (() => {
  ...
  const wbRaw = String(v("wrist.function.weightBear") ?? "");
  const Q8_weightbear = (() => {
