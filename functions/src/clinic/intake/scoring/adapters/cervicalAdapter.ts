...
/**
 * Legacy answer shapes expected by processCervicalAssessment.ts
 */
type LegacyAnswer =
...
export function buildCervicalLegacyAnswers(
  ...
  const painLocationLegacy = (opt: unknown): string => {
    ...
    if (s === "painLocation.oneSide") return "one_side";
    if (s === "painLocation.bothSides") return "both_sides";
    return "central";
  };
  ...
  return [
    ...
    single(
      "C_rf_paresthesia_post_incident",
      yesNo(v("cervical.redflags.paresthesiaPostIncident"))
    ),
    single(
      "C_rf_unable_walk_immediately",
      yesNo(v("cervical.redflags.unableWalkImmediately"))
    ),
    single(
      "C_rf_immediate_neck_pain",
      yesNo(v("cervical.redflags.immediateNeckPain"))
    ),
    single(
      "C_rf_rotation_lt45_both",
      yesNo(v("cervical.redflags.rotationLt45Both"))
    ),
    ...
    single(
      "C_rf_neuro_deficit",
      yesNo(v("cervical.redflags.widespreadNeuroDeficit"))
    ),

    single(
      "C_rf_bladder_bowel",
      yesNo(v("cervical.redflags.bowelBladderChange"))
    ),
    ...
    single(
      "C_morning_stiff_30",
      yesNo(v("cervical.redflags.morningStiffnessOver30min"))
    ),
    single(
      "C_visual_disturbance",
      yesNo(v("cervical.redflags.visualDisturbance"))
    ),

    // ✅ FIX: match your actual canonical questionId
    single(
      "C_gait_unsteady",
      yesNo(v("cervical.redflags.balanceOrWalkingIssues"))
    ),
    ...
    single(
      "C_pain_location",
      painLocationLegacy(v("cervical.symptoms.painLocation"))
    ),
    single(
      "C_pain_into_shoulder",
      yesNo(v("cervical.symptoms.painIntoShoulder"))
    ),
    ...
    single(
      "C_cough_sneeze_worse",
      yesNo(v("cervical.symptoms.coughSneezeWorse"))
    ),
    single(
      "C_neck_movt_worse",
      yesNo(v("cervical.symptoms.neckMovementWorse"))
    ),
    ...
    single(
      "C_headache_one_side",
      yesNo(v("cervical.symptoms.headacheOneSide"))
    ),
    single(
      "C_headache_worse_with_neck",
      yesNo(v("cervical.symptoms.headacheWorseWithNeck"))
    ),
    single(
      "C_headache_better_with_neck_care",
      yesNo(v("cervical.symptoms.headacheBetterWithNeckCare"))
    ),
  ];
}
