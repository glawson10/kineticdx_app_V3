...
/**
 * Legacy answer shapes expected by processElbowAssessment.ts
 */
type LegacyAnswer =
...
export function buildElbowLegacyAnswers(
  ...
  const onsetToTS = (onset: unknown): string => {
  ...
  const locToTS = (loc: unknown): string => {
  ...
  const aggsToTS = (xs: unknown): string[] => {
    const arr = Array.isArray(xs) ? xs.map(String) : [];
    const out: string[] = [];

    const has = (id: string) => arr.includes(id);

    if (has("aggs.palmDownGrip")) out.push("palmDownGrip");
    if (has("aggs.twistJar")) out.push("twistJar");
    if (has("aggs.palmUpCarry")) out.push("palmUpCarry");
    if (has("aggs.overheadThrow")) out.push("overheadThrow");
    if (has("aggs.restOnOlecranon")) out.push("restOnOlecranon");
    if (has("aggs.pushUpWB")) out.push("pushUpWB");

    // If "none" selected, return empty set (it should override others)
    if (has("aggs.none")) return [];
